...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayResults(data) {
        ...
        // --- NEW CODE START: Populate Combined Tags ---
        if (data.tags && data.tags.length > 0) {
            const tagsContent = document.getElementById('tags-container');
            tagsContent.innerHTML = ''; // Clear previous tags
        ...
        if (data.results.gemini) {
            ...
            // 1. Caption (Plain text is fine, or use Markdown if you want)
            ...
            summaryEl.innerHTML = marked.parse(gemini.summary || 'N/A');

            // 3. Detailed Description -> Markdown
            ...
            document.getElementById('gemini-detailed-description-content').innerHTML = marked.parse(detailedDescription);

            if (sdPrompts) {
                // Keep Prompts as text (better for copying)
            ...
            // 4. Hidden Details -> Markdown (Simplified logic!)
            ...
            if (gemini.hidden_details && gemini.hidden_details !== 'No specific hidden details noted.') {
                // Let marked.js handle the bullet points from the backend
                hiddenDetailsEl.innerHTML = marked.parse(gemini.hidden_details);
            } else {
