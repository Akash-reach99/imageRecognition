...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // vvvv NEW GENERATION SELECTORS vvvv
    ...
    // OCR Modal Elements (UPDATED)
    const modalOcrSection = document.getElementById('modal-ocr-section');
    const modalOcrAnnotatedImage = document.getElementById('modal-ocr-annotated-image'); // <-- NEW
    ...
    galleryCards.forEach(card => {
        card.addEventListener('click', () => {
            ...
            const isGenerated = card.dataset.isGenerated === 'true'; // <-- NEW
            
            let predictions = {}; 
            try { predictions = JSON.parse(card.dataset.predictions); } catch (e) { console.error("Failed to parse predictions:", e); predictions = { error: "Could not load details." }; }

            // Populate basic info (unchanged)
            modalImage.src = imageUrl; 
            modalTimestamp.textContent = isGenerated ? `Generated: ${timestamp}` : `Analyzed: ${timestamp}`; // <-- MODIFIED

            // Populate Tags (unchanged)
            ...
            // --- vvvv NEW LOGIC: SHOW MODAL BASED ON TYPE vvvv ---
            if (isGenerated) {
                // This is a GENERATED image. Show simple info.
                modalGenPrompt.textContent = predictions.prompt || "No prompt saved.";
                modalGenInfoSection.classList.remove('hidden');
                
                // Add a simple 'Generated' tag
                const tagLink = document.createElement('a'); 
                tagLink.className = 'tag tag-gemini'; // Use accent color
                tagLink.textContent = 'AI Generated'; 
                tagLink.href = '#';
                modalTagsContainer.appendChild(tagLink); 

            } else {
                // This is an ANALYZED image. Show full details.
                ...
                if (predictions.metadata) {
                    ...
                    for (const sectionTitle in metadata) {
                        if (sectionTitle === 'gps_info') continue; // Skip special key
                ...
                if (predictions.gemini) {
                    ...
                    modalGeminiSummary.textContent = gemini.summary || '';
                    ...
                    modalGeminiDetailed.textContent = detailedDescription || 'N/A'; 
                    if (sdPrompts) {
    ...
    if (zoomModal) { document.addEventListener('click', (e) => { const zoomableTarget = e.target.closest('.zoomable'); if (zoomableTarget) { if (zoomableTarget.closest('#history-modal-content')) zoomModal.style.zIndex = "4000"; else zoomModal.style.zIndex = "3000"; zoomImg.src = zoomableTarget.src; zoomModal.classList.remove('hidden'); } if (e.target.id === 'zoom-modal-backdrop') { zoomModal.classList.add('hidden'); zoomImg.src = ''; } }); }
